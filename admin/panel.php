...
function checkAdminStatus($session, $conn)
{
    if (!isset($session["id"])) {
        header("Location: ../index.php?message_id=" . urlencode('ADM: Id в текущей сессии не найден'));
        exit();
    }

    $specificUserId = $session["id"];
    ...
    if ($result->num_rows === 0) {
        header("Location: ../index.php?message_adm=" . urlencode('ADM: Вы не являетесь администратором.'));
        exit();
    }
...
// Usage
checkAdminStatus($_SESSION, $conn);
        ...
        <?php while ($row = $result->fetch_assoc()) { ?>
            <tr>
                <form method="post" action="update_product.php">
                    ...
                    <td><input type="text" class="form-control" name="product_image_link"
                               value="<?php echo $row["product_image_link"]; ?>"></td>
                    <td><input type="text" class="form-control" name="product_code"
                               value="<?php echo $row["product_code"]; ?>"></td>
                    <td>
                        <button type="submit" name="save" value="<?php echo $row['id']; ?>" class="btn btn-primary">
                            Save
                        </button>
                    </td>
                    <?php
                ...
                <form method="post" action="delete_product.php">
                    <td>
                        <button type="submit" name="pid" value="<?php echo $row['id']; ?>" class="btn btn-danger">
                            Delete
                        </button>
                    </td>
                </form>
            ...
            <?php while ($row = $result->fetch_assoc()) { ?>
                <tr>
                    <form method="post" action="update_users.php">
                        ...
                        <td><input type="text" class="form-control" name="email" value="<?php echo $row["email"]; ?>">
                        </td>
                        ...
                        <td>
                            <input type="radio" id="adminRole" name="role"
                                   value="admin" <?php echo ($row["role"] === "admin") ? 'checked' : ''; ?>>
                            <label for="adminRole">Admin</label>
                            <input type="radio" id="userRole" name="role"
                                   value="user" <?php echo ($row["role"] === "user") ? 'checked' : ''; ?>>
                            <label for="userRole">User</label>
                        </td>
                        <td>
                            <button type="submit" name="save" value="<?php echo $row['id']; ?>" class="btn btn-success">
                                Save
                            </button>
                        </td>
                    </form>
